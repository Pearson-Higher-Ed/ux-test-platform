...
import standAlone.standAlonePageObjects.AvatarDisplayPageObjects;
...
public class AvatarDisplayTest extends BaseClass {
    ...
    @Test(testName = "elementId config Test", dataProvider = "elementId config Test Data", groups = {"desktop-regression", "standAlone"})
    ...
    private void avatarURLTextConfigTest(String configType, String[] configArray, String[] avHeight, String[] avWidth) throws Exception {
        ...
        if (!isHeight) {
            log.info("Height for avatar for config type " + configType + " is not as per the spec, actual: "+height);
        }
        isWidth = commonUtils.assertCSSProperties(configType, width, avWidth);
        if (!isWidth) {
            log.info("Width for avatar for config type " + configType + " is not as per the spec, actual: "+width);
        }
        commonUtils.writeInitialConfig(tempJSFilePath, avatarDisplayJSPath);
    ...
    @Test(testName = "Mobile: avatar shape Test", dataProvider = "Avatar Shape Test Data", groups = {"mobile-regression", "standAlone"})
    ...
    @Test(testName = "Mobile: avatarURLText config Test", dataProvider = "avatarURLText config Test Data", groups = {"mobile-regression", "standAlone"})
    ...
    @Test(testName = "Mobile: avatarALTText config Test", dataProvider = "avatarALTText config Test Data", groups = {"mobile-regression", "standAlone"})
    ...
    @Test(testName = "Mobile: avatarSize config Test", dataProvider = "avatarSize config Test Data", groups = {"mobile-regression", "standAlone"})
