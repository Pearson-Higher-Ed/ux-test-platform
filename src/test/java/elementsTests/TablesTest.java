...
import elements.elementsPageObjects.TablesPageObjects;
...
public class TablesTest extends BaseClass {
    ...
    private final String url = "http://localhost:8000/src/main/java/elements/fixtures/tables.html";
    ...
    // Find header elements in the table and check its CSS properties
    ...
    private void checkBoxPropsMobileTest() {
        ...
        for (int i = 1; i <= listElements.size() - 1; i++) {
            ...
            isWidth = commonUtils.assertCSSProperties("width", width, new String[]{"19px", "19.01px"});
            if (!isWidth) {
                log.info("width of checkbox is not as per spec, actual: " + width);
            }
            isVerticalAlign = commonUtils.assertValue(verticalAlign, "top", "vertical align of checkbox is not as per spec");
